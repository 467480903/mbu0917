...
主要有以下数据类型，和对应的数据库表格：关节角 坐标姿态 地图点位

关节、位姿 和 地盘控制器， 里面都不要引用 数据库控制器， 相反的， 数据库控制器里要引用这3个控制器， 

没一类数据， 都要实现 增  删  改  读取所有  运动到这个位置   的功能，不需要  查询 功能，数据一律按 名称 做字符排序，

特别注意， 对于  运动到位置 这个功能， 不要写自动测试功能， 因为我人不在机器旁边， 你直接跑测试会导致设备撞击。

## 如何测试

首先，要有一个服务启动程序， 加载这个控制器， 并启动这个控制器服务的功能，应该是个 Python 文件， 它将执行多个 MQTT 的 PUB 和 SUB 功能

我们的最终的服务， 会把所有的控制器都加载进去， 一个服务中启动多个控制器， 但是在做一些功能测试的时候， 只需要加载对应的控制器

要依次测试3类数据的  增 删 改 读取 功能，   运动到位置 这个功能， 程序功能要有， 但是不要写自动化测试

所谓自动化测试， 也就是创建多个  .py 类程序， 然后里面跑 assert 验证功能， 

然后还要创建简易观察类程序， 也就是一些 .sh 文件， 我可以直接跑这些文件， 来观察命令行上的输入输出

## 进一步的人机交互测试

在目录 web/test/database  中， 创建一些功能单一的网页, 只要  .html 的文件， 不要创建 额外的 .js 文件， js 脚本放在 .html 文件里面， 可以实现各类数据的增删改功能。 网页上可以把  运动到位置  这类功能按钮加上 ， 方便我人工手动触发按钮来做测试
